...
WATERMARK_STROKE_COLOR = (0, 0, 0, 220)  # Чёрный, 47% непрозрачности
...
# Коэффициент перекрытия для плотной заливки (dense tiling)
# 0.8 = 20% перекрытия прозрачных областей bounding box
OVERLAP_FACTOR = 0.8
...
def get_font_size(width: int, height: int) -> int:
    ...
    Правила:
    - Если ОБА измерения > 1000: фиксированный размер 50px
...
def add_watermark(image_bytes: bytes, watermark_text: str = None) -> BytesIO:
    ...
    - Фиксированный размер 50px для изображений > 1000x1000
    - Плотная заливка (dense tiling) с перекрытием bounding boxes
    ...
    # ============================================================
    # Шаг 4: Расчёт spacing с перекрытием для плотной заливки
    # ============================================================
    # Bounding box повёрнутого текста содержит много пустого пространства
    # по углам. Чтобы текст визуально был близко, накладываем их
    # друг на друга с коэффициентом перекрытия.
